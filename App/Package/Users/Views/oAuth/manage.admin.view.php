...
<div class="page-title">
    ...
    <div>
        <a type="button" class="btn-success"
           href="https://craftmywebsite.fr/docs/fr/users/oauth-connexion-externe/explication" target="_blank"><i
                class="fa-solid fa-up-right-from-square"></i> Explications</a>
            ...
            if (!UsersOAuthModel::getInstance()->isMethodEnabled($implementation->methodIdentifier())):
                ...
                <div class="card">
                    <div class="flex flex-row justify-between mb-2">
                        ...
                        <label class="toggle" style="align-self: center">
                            <input id="toggle_<?= $methodId ?>" type="checkbox"
                                   name="oauth_enabled[<?= $implementation->methodIdentifier() ?>]"
                                   class="toggle-input" value="1"
                                <?= UsersOAuthModel::getInstance()->isMethodEnabled($implementation->methodIdentifier()) ? 'checked' : '' ?>>
                            <div class="toggle-slider"></div>
                        </label>
                    </div>
                    <div id="form_<?= $methodId ?>"
                         style="display: <?= UsersOAuthModel::getInstance()->isMethodEnabled($methodId) ? 'block' : 'none' ?>;">
                        <?php $implementation->adminForm() ?>
                    </div>
                </div>
            <?php endif;
